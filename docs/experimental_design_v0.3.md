...
#### Model H: 群れ(herding、原典: Kirman 1993 / Lux-Marchesi 1999 / Alfarano-Lux-Wagner 2008)

T と土台を揃え、speculative 成分のみ trend → herd に差し替えた混合(母集団比率 β=(β_h, β_f, β_n)、herder 優勢):

- **herder**(集約行動を読む): `social_{i,t} = mean(ā_{t-h_i^s:t})`、`ā_τ = (1/N) Σ_j a_{j,τ-1}`。`a = sign(social)`(多数派に同調)。
- **fundamentalist**(誤価格を読む、**錨**): T と同形。`a = sign(m) if |m|>θ_i else 0`。
- **noise**: 一様ランダム(自走の素)。

> **v0.3 設計判断**: 純 herding(錨なし・行動を直接コピー)は単調暴走し SF が出ない。Kirman/ALW は fundamentalist の錨を持ち、herding は opinion を溜め fundamentalist が引き戻す → 間欠的な大変動(fat tails/ボラ集中)。よって fundamentalist 錨を入れた混合にする(原典に忠実)。smoke で純 herding が爆発、錨ありで有界化を確認済み。

ヘテロ性: 成分割当 ~ Categorical(β)、`p` 系は herder の同調を将来パラメータ化、`h_i^s ~ DiscreteUniform[5, 50]`、θ_i(fund)~ Uniform[0, θ_max]。
Free parameters (calibration target): β, σ_n, θ_max, h^s の範囲。

> **機構の差(T vs H)** = speculative 成分が読む観測: 価格トレンド(T.chartist)か他者の行動(H.herder)か。trend masking は T を、social masking は H を叩く。fundamental masking は両者の fundamentalist を叩く(共有=対照軸)。**弁別は主に trend/social masking の非対称が担う**(§7.2)。
...
| 集約行動履歴 | L = 100 | ā_{t-L:t}(社会信号の raw source) | H.herder | social masking |
| fundamental 観測 | series | `m_τ = log(p*/p_τ)`(誤価格系列、§3.1。baseline clean、σ_f は B2 が担う) | T/H.fundamentalist | fundamental masking |
...
| Social masking | 集約行動履歴 ā_{t-L:t} を degrade | Model H(herder) |
| Fundamental masking | 誤価格系列 m_τ を degrade | T/H の fundamentalist(共有・対照) |

各軸を独立に動かす(現状 3D intervention space、拡張可能)。**弁別の主軸は trend masking(T の chartist を叩く)と social masking(H の herder を叩く)の非対称**: T は trend masking に強く応答し social masking に鈍い、H はその逆、という応答パターンの差が弁別シグナル。fundamental masking は両機構の fundamentalist を等しく叩くので対照軸(両者が同程度応答 → 機構非依存経路の確認に使える)。
