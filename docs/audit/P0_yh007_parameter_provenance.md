...
- §4 項目 1(頑健性再走の構成復元不能)は**未解消のまま**。修正値での再走は
  主張値と表示桁一致したが、対応のある差(seeds 共通、n=8)で検定すると
  ret_acf チャネルは zi_matched(t=−1.51)も E_W0(t=−0.37)も較正を統計的に
  識別しない — 点推定の一致は識別根拠にならない。決定的に φ/σ 感応な
  チャネルは agg(paired t=−65)だが、元の主張は agg を記録していないため
  遡及識別に使えない。決定論性(全桁一致)に基づく状況証拠はあるが
  「同 seeds」の補助仮定に依存するため解消根拠としない。
