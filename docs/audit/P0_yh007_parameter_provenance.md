...
- §4 項目 1(頑健性再走の構成復元不能)は、修正値での再走が主張値を表示桁まで
  再現し、**かつ旧値対照 run(同一コード・同一 seeds)が主張値を再現しない**
  ことの両方をもって解消(一致が構成を識別することを対照で確認済み)。
- §4 項目 5(agg parity 参照値 0.102 の由来 run)も対照 run の zi_matched
  agg = 0.102 の一致により解消(旧較正 run 由来と確定)。
- 対になる数字: 上記により `P0_rerun_yh007_8_p3d.json` が **YH007-8 系で
  最初の一次 artifact**(0 件 → 2 件、対照 JSON 含む)。
- 詳細: `docs/audit/P0_yh007_recalibration_rerun.md`。残タスクは
