...
- **zi_matched も識別を担えない**。点推定は −0.054 → −0.094 と 74% 動くが
...
(注文フロー段階 → 価格段階)に沿って測定 S/N が約 2 桁落ちることを示す —
...
- 両較正で保持されるのは**符号構造**(D1_W0 ≈ 0 / E_W0 ≪ 0 の対比)のみ。
  係数の大きさについては、n=8 では「変わらない」とも「74% 変わった」とも
  断定できない(上表)。「較正修正に頑健」はこの弱い意味で読むこと。
