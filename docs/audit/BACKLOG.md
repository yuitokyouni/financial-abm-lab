...
8. 8/19 対照表(20 項目)が**どの ref にも一度もコミットされていない**
   ...
   **2026-08-23 に部分代替を収容**: `W1D4_schema_table_reconstruction.md`
   (repo 状態から機械的に導出できる列だけで再構成。収容時に全導出主張を機械検証)。
   **逐語原本ではなく、本項目を解消しない。** 原本 20 行と凍結内容の照合は
   transcript 回収が要る。原本パス `sieve/docs/contract/schema.md` は
   回収時の受け皿として空けたまま。
