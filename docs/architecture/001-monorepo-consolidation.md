...
- **Stage B 進行中 (`stage-b-sg-backbone` ブランチ、OneDrive 同期済 / GitHub 未 push=トークン要):**
  ...
  - experiments: `experiments/YH005/baseline.py` と `experiments/classical/baseline.py`
...
| YH005 / YH005_1 (SG) | ✓ core import | `experiments/YH005/baseline.py` |
...
| 旧リポ | 移植先 | 主資産 |
|---|---|---|
| speculation-game-info | `experiments/classical/`, `experiments/YH005/` + 抽出を `packages/` | YH001-006_1 (17k LOC) |
...
- **T0. Backbone 検証** ← 最初の一歩 / go-no-go ゲート
  ...
  - T0.5 結果を `experiments/YH005/docs/backbone_parity.md` に記録 (go-no-go 判定)
...
この文書は統合の経緯・元の受入条件を保持する。現在のファイル配置と所有範囲は
[repository-layout.md](repository-layout.md)を参照。古い配置図は現在の実装一覧ではない。
