...
両モデルの 6 次元 SF feature 分布が、両 SF classifier で区別不能(accuracy 50-55%)になるパラメータ点を見つける。

### 5.2 探索アルゴリズム

1. **Stage 1: 個別 calibration**
   - Model T: λ, h_i 範囲, θ_i 範囲を Bayesian Optimization (Optuna) で動かし、6 次元 SF feature が経験データ(S&P500 1990-2020 日次 returns)に近いパラメータ点を探す
   - Model H: 同様
   - 各 500 trial、Gaussian Process surrogate
2. **Stage 2: 相互距離最小化**
   - Stage 1 のパレートフロントから T 側 / H 側それぞれ k=10 個の候補点を抽出
   - 全 10×10 ペアで M=200 runs ずつ SF feature 分布を生成
   - 6 次元 SF feature 空間での Wasserstein 距離を全ペアで計算
   - 距離最小のペア (T*, H*) を選択
3. **Stage 3: 等価性事前検証**
   - (T*, H*) で M=1000 runs ずつ生成、両 SF classifier で 5-fold CV accuracy を計算
   - 50-55% に入らなければ Stage 1-2 をやり直し、または SF battery 修正(=主結果のスコープ縮小として明示記録)
...
- 入力: 6 次元 SF feature vector (SF1-SF6)
...
3. **SF battery 依存**。SF1-SF6 で等価でも、他の SF (Epps effect、market impact 関数、intraday seasonality 等) を含めると等価性が崩れる可能性。pre-register された結論は「この 6 SF battery 上で等価な (T, H) は IR で分けられる」まで。
...
- v0.1 (2026-06-05): 初版 draft、PRISM 撤退判定後の真・PRISM toy 設計