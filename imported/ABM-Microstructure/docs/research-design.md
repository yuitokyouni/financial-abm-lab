...
## 9. 改訂メモ（v0.3, 2026-06-11）— 主従の入れ替え：移植可能性監査を主成果物に

研究プログラム全体（P1 Atlas コア / P2 本研究 / P3 LLM-ABM 監査、`PROV-ABM-atlas/docs/program_claims_v1.md`）への接続に伴う再フレーミング。**設計変更ではない**——finding 0002 とdensity spoke は既に「帰結がどちらでも主張が立つ」構造で事前登録済みであり、変わるのは主張の主従と robustness 予算の配賦先。

### 9.1 P2 目標文（この一文を否定不能にする）

> **Calvano 型アルゴリズム共謀の成立は、毎期・決定論的報酬という Bertrand 環境の特殊性に依存する。現実的な市場 making の疎・高分散報酬構造に移植すると、(i) Calvano の収束概念（全状態 policy 安定）は構造的に到達不能になり、(ii) 行動レベル（greedy limit-cycle）で認定可能な共謀 regime は事象密度 × 学習率空間の特定領域に限られる——あるいは tabular 予算内に存在しない。実在 venue（BCS ES–SPY 較正点）はこの空間の疎側に位置する。**

- 対象権威：EC / Management Science / JEDC、および JPX・日銀（FBA vs CLOB・speed bump は Budish 以来の現実の政策論争で、JPX は tick size / HFT 規制で人工市場研究を実際に使ってきた）。
- 主張の型：**移植可能性監査（portability audit）**。「Calvano（AER）の手順は orderbook に持ち込むと何が壊れ、どこなら成立するか」を、検証済み harness（実験A license）＋事前登録した収束基準・認定 gate の下で確定する。これは P3（他者の LLM-ABM 監査）に先立つ「自分の手で監査をやって見せた実績」になる。
- 最後の一節（BCS 較正点の位置づけ）は US4 の役割昇格：較正セルは外部妥当性の飾りではなく、**監査結論の現実への接地**（「現実の ES–SPY パラメータは認定可能 regime の内か外か」）。

### 9.2 主従

- **主**：Calvano 移植可能性監査（§9.1）。finding 0002（収束概念の崩壊と再設計）＋ coarse 非収束地図＋ density spoke の認定境界＋ BCS 較正点の位置、が本体。
- **従**：latency-fairness × collusion-resistance の設計マップ統合（旧 §0 の中心問題）。批准可能 regime が見つかった場合に、その regime 上で batch 変調（Δ_GP / Δ_pred 帰属）を主張する。認定ゼロの場合、従は「変調は分布的時間平均の方向示唆に留まる」と正直に縮む——主（監査）は regime 不在でも立つ。

### 9.3 帰結（spec への反映事項）

1. **robustness tier（D-B9 Tier-3）の配賦先改定**：headline 点は「認定通過点」前提だった。主従入れ替え後、headline = **density spoke の認定セル**（存在すれば）。よって Tier-3 予算は density regime 上で執行する（SARSA・tie=rotate・lr/β/γ 振り・×20 seed）。認定ゼロの場合、Tier-3 は「非収束の頑健性」（基準 v2 でも非収束が algo/HP 横断で再現すること）に充てる——これも監査主張 (ii) の直接の証拠になる。
2. **OSF 事前登録の運用開始**：density spoke の結果が出る**前**に、解釈規則（D-B6 v2・認定 gate・§9.1 の主張構造とその縮退規則）を OSF に公開タイムスタンプ付きで登録する。git 履歴は内部 timestamp として既に機能しているが、P3 で他者を監査する資格の前提として外部登録を習慣化する（プログラム共通要件、program_claims_v1.md §3）。
3. **スコープ規律**：本研究が主張するのは合成（較正済み）環境での移植可能性とregime 同定まで。**実市場データからの共謀検出・機構同定は主張しない**（その境界を越えた一文は書かない）。さらに3つの内側スコープを固定する（finding 0002 Limitations と同文）：(a) 主語は**表形式学習の枠内**——対外的な一文に必ずこの限定を入れ、「現実市場で共謀リスクが低い」と読ませない。(b) ν 軸は報酬・監視・学習統計の**束**——claim (i) は束の主張として書き、成分分離（報酬は密・観測だけ疎の軸）は主張しない。(c) D-B11 弾力需要軸は descope（markup 主張は認定有無と同一 grid 内比較に限定）。
4. **Model Contract への接続**：本 harness（002）は Atlas Model Contract v0 の最初の外部参照アダプタ候補（`PROV-ABM-atlas/docs/model_contract_v0.md`）。契約適合は 002 の scope 外、将来 feature として切る。
