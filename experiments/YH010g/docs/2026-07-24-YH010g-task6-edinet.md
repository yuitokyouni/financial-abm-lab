...
- 日付: 2026-07-24
- 対象: `experiments/YH010g/specs/HANDOFF.md` Task 6（EDINET属性パイプライン）の実装本体と Task 2 の本検証
...
uv run python -m yh010g.build_attributes \
  --targets experiments/YH010g/data/fixtures/validation_targets.csv \
  --out experiments/YH010g/data/processed/attributes_edinet.csv
```
...
著名議案のground truth（`experiments/YH010g/data/fixtures/validation_groundtruth.csv`、Task調査の17件から財務・機械判定可能な7件を抽出）に対し、規則ベース再構成の推奨方向が実推奨と一致するかを **mechanism層別** で測定。
...
2. `uv run python -m yh010g.build_attributes --targets experiments/YH010g/data/fixtures/validation_targets.csv --out experiments/YH010g/data/processed/attributes_edinet.csv`
3. 要素ID未マッチが出たら `python -m yh010g.edinet dump <docID>` で実IDを確認し `edinet.py` の `EL_*` 定数を調整
4. `uv run python -m yh010g.validate_policy --edinet experiments/YH010g/data/processed/attributes_edinet.csv` で financial系ISSが反対を再現することを確認
