...
`002-yh007-kronos-microstructure.md`(同ディレクトリ) の §5 YH007-8 行 + §10-D を展開した
...
- **P2 合格診断**: agent 横断の placement price の **時間平均分散が seed 全体で非ゼロ、かつ時系列で
  変動**する。満たせば §3.2 OK。
- **⚠ P3 で判明した限界(§12 round4)**: この cross-section 分散診断は **時間方向の集合的
  directionality を捕まえない**。quantile-rank は placement の**幅**を散らすが、Kronos 中心が
  drift すると**全 quantile が同符号に shift** → 集合 cross → over-shoot(ret_acf τ=1=−0.23)。
  → dispersion guard に **「per-bar 方向バランス(同側比率)が 100% に張り付かない」**を追加し、
  逆張り流動性(④)で吸収する設計が要る。**P2 の var_ts_std>0 だけでは degeneracy 否定に不十分**。
...
> **訂正(2026-08-13、§12 末尾の訂正節参照)**: 「必要十分条件」という論理式の主張は
> 2×2 の効果分解に基づく主張へ訂正する — **S3(sticky anchor)の主効果はどの推定量・
> どの較正でも有意(層別 t=−5.3〜−7.9、プール t=−8.8/−10.6)。S2 は層別では検出されず、
> プール推定量で修正較正のみ小さな正効果(t=+3.4、旧較正では n.s.)= 示唆止まり**。
> 推定量・較正込みの表の正は §12 訂正節。recenter 採択の根拠(S3 が bounce を駆動し
> S2 単独では生じない)はこの形で存続する。
...
> **明確化と現状(2026-08-13)**: 条件 (ii) の「SF 指標健全」は **Hill α と vol ACF の
> 健全性**を指す。agg parity は (ii) の一部だが**参照値は修正較正の ZI-matched
> (= 0.072、8 seed mean)を用いる** — 2026-07-21 の P3-F 合格は旧較正参照 0.102 に
> 対する parity で判定された**条件付き合格**であり、修正較正下での (ii) 充足は未確認。
...
> **現状(2026-08-13)**: 条件 3 の dose-match は修正較正値 φ=0.615/σ=3.81e-3
> (コード全箇所反映済み)で行い、agg parity の参照は修正較正の 0.072。
> 成功条件の充足判定は修正較正下で取り直すまで確定しない(§7 の P4 保留を参照)。
...
| **P4** | P3 が合格(bounce が ret_acf≈0 に収まる)した構成で headline run。seed/bar 確定済 | real | 3 点同時達成。**P3 合格まで held** → **【2026-08-13】held 継続**: P3-F は旧較正下の条件付き合格(parity 参照 0.102 = 旧較正)で、修正較正(φ=0.615/σ=3.81e-3)での再走・合格が P4 解除の前提(`docs/audit/BACKLOG.md`) |
...
| 日付 | 内容 |
|---|---|
| 2026-08-13 | **φ/σ 更新漏れの修正 + 頑健性再走 + 旧値対照 run**(P0 監査 `experiments/YH007/docs/audit/P0_yh007_parameter_provenance.md` へのユーザ裁定に基づく)。round6 追記の修正値 φ=0.615/σ=3.81e-3 を、旧値 0.418/6e-3 のまま残っていたコード全 9 箇所(model 既定値 / zi_agent fallback / P3 系スクリプト 4 本 / テスト fixture 2 本)に反映し、regression guard(`experiments/YH007/tests/test_yh007_8_calibration_values.py`)を新設。コミット済みコードでの再走は **zi_matched = −0.094±0.06 / E_W0 = −0.260±0.06**(± は 8 seed SD)を表示桁まで再現、旧値対照(同一コード・同一 seeds)は −0.054±0.07/−0.250±0.06。**ただし対応のある差では ret_acf チャネルは較正を識別しない**(zi_matched t=−1.51、E_W0 t=−0.37、いずれも n.s.)— 決定的に感応するのは agg(0.102→0.072、paired t=−65)だが元の主張は agg 未記録のため遡及識別不能。**原 run 構成の判定不能は【復元不能】として恒久クローズ**(元主張は n・seed 構成不明の点推定で、唯一識別可能な agg が元記録に無いため解決経路が存在しない。追加 seed では追わない)。**頑健の内容は「二重乖離の符号構造(D1≈0/E≪0)が両較正で保持される」に限定**され、係数の大きさは n=8 で不定。§3.8 採択は符号構造のみに依存するため較正非依存で存続。**P3-F は旧較正下の条件付き合格(parity 参照 0.102 = 旧較正 zi_matched と確定、修正値では 0.072)で、修正較正下での合格は未確認** — 再走と P4 解除判断は backlog(`docs/audit/BACKLOG.md`)へ。付随発見: 旧 1-group テスト構成は修正値で agg 崩壊(0.0015)するため 2-group dose-match 構成に更新。artifacts: `experiments/YH007/reports/audit/P0_rerun_yh007_8_p3d.json` + `P0_control_oldvalues_yh007_8_p3d.json`。 |
...
### round4 裁定(P3 結果 — bounce は Kronos 戦略構造由来、substrate 未完成、2026-06-23)

**P3 FAIL を支持。β の discipline を評価**(discriminator を回し「herding だから OK」に逃げず
ret_acf 超過=不合格と正しく判定)。結果の確定事項:
- **ZI-matched −0.054(合格)/ CI×Kronos −0.228(超過)**。同流動性・同 agg・同 AR(1) 較正で
  ZI-matched は出ない → **機械的 microstructure artifact(薄板 sawtooth)ではなく、Kronos 戦略構造由来**。
- 形状: τ=1 spike + τ=2≈0 = over-shoot。vol_acf τ=1 で kronos>zi(機構 3 が弱く出かけ)。

**裁定 1 — substrate は未完成。P4 held を継続**。「artifact でなく Kronos 由来」は
「だから feature/OK」を**意味しない**(round3 の規律を継続)。−0.228 は bar スケールで Cont(2001)
SF#1「線形自己相関は ~0」を破る規模。**ret_acf≈0 は合格必須のまま**。

**裁定 2 — 根因の精緻化(β の「全 quantile が同方向 translate」を一段深める)**:
quantile-rank は **cross-section の幅**を散らすが、**時間方向の集合的 directionality** は散らさない
(中心が drift すると全 quantile が同符号 shift → 集合 cross → over-shoot)。さらに疑うべきは
**戦略 mix が ~all-trend に collapse**(YH007-3 で trend 98% 例)している可能性 — その場合
**逆張り(fade)流動性が枯渇**して trend 群の aggression を吸収できず over-shoot する。これは
**④(淘汰が逆張り比率を内生維持、002 §4.4)が機能していない**こと = §3.2 dispersion guard が
この failure mode に**無効**だったことを意味する。

**裁定 3 — 診断順序**:
- **【最優先・logs から cheap】(d) + 戦略 mix readout**: ① per-bar の方向バランス(各 bar で
  何 % の agent が同じ側か)② Kronos 中心 drift の大きさ ③ **trend/fade 構成比**。
  「shared 信号 → 集合的 directional shift + fade 枯渇」仮説を直接検証。
- **(c) 板密度感度は後回し・再フレーム**: ZI-matched で artifact は既にほぼ除外済なので「artifact
  vs degeneracy」の切り分けには冗長。価値は「吸収流動性を増やすと over-shoot が減衰するか」=
  **候補 fix の検証**。(d) を読んでから、必要なら回す(1.5h を先に払わない)。

**裁定 4 — 候補 fix(d 確認後に選定、今は選ばない)**:
(i) **逆張り流動性の回復**(④ が逆張りニッチを維持しない理由を調査。over-shoot 時に fade が
儲かるはずなのに選択されないなら payoff window / 参加ゲートを見直す)、(ii) 反応 lag/horizon の
異質化(desync、ただし per-agent 信号で compute 増)、(iii) bar あたり集合 aggressive 参加の cap。

**裁定 5 — これは「欠陥」であると同時に「候補 finding」**: 「**同一基盤モデルを全 agent が共有
すると directional 同期 → 集合的 over-reaction(return reversal)**」は、foundation-model 駆動
...
### 訂正・現状整理(2026-08-13、P0 監査の是正に基づく)

上の round6 裁定・追記に対する訂正 3 点(元文は履歴として残す。一次 artifact:
`experiments/YH007/reports/audit/P0_rerun_yh007_8_p3d.json` + `P0_control_oldvalues_yh007_8_p3d.json`、
± は 8 seed SD(ddof=1)、対応差の SE = SD(diff)/√8、df=7):

1. **「bounce の必要十分条件 = sticky anchor(S3)単独」の主張の型を訂正**。
   2×2(S2 共有 × S3 sticky)の**対応のある差**で言えるのは以下であり、
   論理式「必要十分」より弱いがセルの点推定比較より強い:

   **推定量を明記する(層別とプールで結論が変わるため、以下の表が正)**:
   「層別」= 各層のセル対比(seed 対応差、n=8)。「プール」= seed ごとに
   2 層の対比を平均した per-seed 平均(n=8。交互作用ゼロを仮定 — 交互作用行は
   その検査で、支持される)。

   | 効果(ret_acf[1]) | 推定量 | 修正較正 | 旧較正 |
   |---|---|---|---|
   | S3 主効果(S2 なし層: E_W0 − zi_matched) | 層別 | −0.166, SE 0.031, **t=−5.3** | −0.196, SE 0.029, **t=−6.7** |
   | S3 主効果(S2 あり層: D2_W0 − D1_W0) | 層別 | −0.168, SE 0.025, **t=−6.7** | −0.204, SE 0.026, **t=−7.9** |
   | S3 主効果 | プール | −0.167, SE 0.019, **t=−8.8** | −0.200, SE 0.019, **t=−10.6** |
   | S2 主効果(S3 なし層: D1_W0 − zi_matched) | 層別 | +0.038, SE 0.024, t=+1.6 (n.s.) | +0.018, SE 0.035, t=+0.5 (n.s.) |
   | S2 主効果(S3 あり層: D2_W0 − E_W0) | 層別 | +0.036, SE 0.023, t=+1.5 (n.s.) | +0.010, SE 0.022, t=+0.4 (n.s.) |
   | S2 主効果 | プール | +0.037, SE 0.011, **t=+3.4** | +0.014, SE 0.021, t=+0.6 (n.s.) |
   | 交互作用 | — | −0.003, SE 0.042, t=−0.1 (n.s.) | −0.008, SE 0.041, t=−0.2 (n.s.) |

   **結論(推定量ごとに読み分けること)**:
   - **S3 主効果はどの推定量・どの較正でも負・有意**(プール点推定 −0.167 /
     −0.200。S2 のプール点推定は +0.037 / +0.014)。**較正間の S3 主効果の差は
     検出されなかった**(contrast +0.033、対応 SE 0.019 で t=+1.7 /
     独立 SE 0.027 で t=+1.2、いずれも n.s. — n.s. は「同じ」ではなく
     「区別できない」を意味する)。recenter(§3.8)採択の実質的根拠は
     この形で存続する。
   - **S2 主効果は層別では両層・両較正で n.s.**。プール推定量では**修正較正
     のみ** +0.037(t=+3.4)の小さな正効果(bounce 減衰方向)が検出されるが、
     旧較正では再現しない(t=+0.6)。プールの SE が層別より小さいのは修正較正で
     2 層の対比が seed 内負相関(r=−0.57)のため。**較正間で再現せず推定量
     依存でもあるので、S2 の正効果は示唆に留め、確立された効果として
     扱わない。**
2. **P3-F は旧キャリブレーション下での条件付き合格**。合格判定 (ii) の agg parity は
   旧較正参照 0.102(= 旧較正 zi_matched の agg と確定)に対して行われた。修正較正の
   参照は 0.072。**修正較正下での P3-F 合格は未確認 → P4 held は継続**(§7)。
3. **上記追記の頑健性再走の原 run 構成は復元不能として恒久クローズ**(記録された
   ret_acf[1] は較正への感応が n=8 で n.s.、識別可能な agg が元記録に無い)。
   数値自体はコミット済みコード + artifact で再現・再確定済み。
   詳細: `experiments/YH007/docs/audit/P0_yh007_recalibration_rerun.md`。
