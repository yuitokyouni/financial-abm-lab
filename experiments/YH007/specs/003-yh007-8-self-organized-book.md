...
| 日付 | 内容 |
|---|---|
| 2026-08-13 | **φ/σ 更新漏れの修正 + 頑健性再走の再確定(対照 run による識別込み)**(P0 監査 `docs/audit/P0_yh007_parameter_provenance.md` へのユーザ裁定に基づく)。round6 追記の修正値 φ=0.615/σ=3.81e-3 を、旧値 0.418/6e-3 のまま残っていたコード全 9 箇所(model 既定値 / zi_agent fallback / P3 系スクリプト 4 本 / テスト fixture 2 本)に反映し、regression guard(`tests/test_yh007_8_calibration_values.py`)を新設。復元不能だった頑健性再走はコミット済みコードで再実行し **zi_matched = −0.094±0.06 / E_W0 = −0.260±0.06 を表示桁まで再現**、さらに**旧値対照 run(同一コード・同一 seeds)は −0.054/−0.250 となり主張値を再現しない = 一致が構成を識別することを確認**(artifacts: `docs/audit/P0_rerun_yh007_8_p3d.json` + `P0_control_oldvalues_yh007_8_p3d.json`、詳細: `docs/audit/P0_yh007_recalibration_rerun.md`)。二重乖離の符号構造(D1≈0 / E≪0)は両較正で保持 = 頑健性を再確定。対照の zi_matched agg=0.102 により P3-F の parity 参照値の由来も旧較正 run と確定。付随発見: 旧 1-group テスト構成は修正値で agg 崩壊(0.0015)するため 2-group dose-match 構成に更新。**P3-F の修正値再走と P4 解除判断は backlog(`docs/audit/BACKLOG.md`)へ**。 |
