...
> 2×2 の効果分解に基づく主張へ訂正する — **S3(sticky anchor)の主効果のみが有意
> (両 S2 層・両較正で t=−5.3〜−7.9)、S2 の主効果と交互作用は n=8 で検出されない**。
> recenter 採択の根拠(S3 が bounce を駆動し S2 単独では生じない)はこの形で存続する。
...
1. **「bounce の必要十分条件 = sticky anchor(S3)単独」の主張の型を訂正**。
   ...
   | 効果(ret_acf[1]) | 修正較正 | 旧較正 |
   |---|---|---|
   | S3 主効果(S2 なし層: E_W0 − zi_matched) | −0.166, SE 0.031, **t=−5.3** | −0.196, SE 0.029, **t=−6.7** |
   | S3 主効果(S2 あり層: D2_W0 − D1_W0) | −0.168, SE 0.025, **t=−6.7** | −0.204, SE 0.026, **t=−7.9** |
   | S2 主効果(S3 なし層: D1_W0 − zi_matched) | +0.038, SE 0.024, t=+1.6 (n.s.) | +0.018, SE 0.035, t=+0.5 (n.s.) |
   | S2 主効果(S3 あり層: D2_W0 − E_W0) | +0.036, SE 0.023, t=+1.5 (n.s.) | +0.010, SE 0.022, t=+0.4 (n.s.) |
   | 交互作用 | −0.003, SE 0.042, t=−0.1 (n.s.) | −0.008, SE 0.041, t=−0.2 (n.s.) |

   **S3 の主効果のみが両層・両較正で有意。S2 の主効果と交互作用は n=8 で
   検出されない。** recenter(§3.8)採択の実質的根拠はこの形で存続する。
