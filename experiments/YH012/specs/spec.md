...
2026-09-06: 修正の commit/push 後、適格39 seed を end_time=50,000 のまま再実行した。
全件で介入前の生バイト一致を確認し、保存した F/B から平均・標準偏差・信頼区間を算出した。
初期窓の平均は −3.8715、終盤の時間平均は −6.8814 ticks（95% CI [−20.9606, +0.3487]）。
特に適格な seed 14 の大きな価格経路の分岐が平均を支配した。追加除外はしていない。
[結果と元ログ](../reports/ensemble_q200_eligible39/README.md) を参照。
