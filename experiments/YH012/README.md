...
この未完了の ensemble は平均せず、図・Δ∞ 推定値を出さなかった。
[停止記録と元ログ](reports/ensemble_q200_attempt1_stopped/README.md) を参照。
その後の診断で、固定時刻 t0 に best_ask がない候補を事前に除外する方針を採用した。
保存済みネイティブ気配では seed 13 のみが該当し、適格標本は39 seed。
発注ロジック・t0・背景パラメータは維持する。測定対象は t0 に売り板がある背景市場での条件付き平均。
lobcore のパディング修正と厳密な往復テストを通した後、新しい F/B ペアで再実行する。

実行・集計コマンド（診断の元気配・背景ログ・メタデータがローカルに必要）:
...
.venv/bin/python -m experiments.YH012.run_ensemble \
  ...
  --liquidity-diagnostic experiments/YH012/artifacts/liquidity_diagnostic_20260906
...
除外は番号によらず、保存済みの t0 のネイティブ ask 数量から判定する。
診断の気配・背景ログのチェックサム、実験設定、背景コードを照合し、
候補40件の観測値・適格性・除外理由・元データのハッシュを `plan.json` に保存する。
適格性診断を指定しない場合は、従来どおり全候補を実行して欠損時に停止する。
...
終盤 [45,000,50,000] は各適格 seed の時間平均を求め、その39個を再標本化する。
...
総約定量は発注時の taker 分と、残った指値が後から約定する maker 分を合計する。
今回の原本 summary.json に残る旧 taker のみの数量欄と、訂正後の集計値の違いはレポートに記録した。
