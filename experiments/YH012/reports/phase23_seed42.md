...
当初使用した lobcore の `logs_byte_equal` は NumPy の構造化配列のフィールド比較で、
...
2026-09-06 追記: lobcore `6aff004` でパディングの初期化と同ヘルパーの厳密なバイト比較を修正した。
この seed 42 の F/B を修正後に再実行し、介入前1,974,720バイトだけでなく、
各ログ本体の全バイトが過去の保存値と一致することを確認した。
上記の結果は有効であり、当初のヘルパー単独の検証は「フィールド一致」として区別する。
