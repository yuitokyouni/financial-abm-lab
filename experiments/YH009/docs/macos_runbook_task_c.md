...
```bash
python3 -m pytest experiments/YH009/tests/test_car_engine.py -v
```
...
```bash
python3 experiments/YH009/scripts/jquants_fetch.py
```
...
一部だけ再取得したいとき:
```bash
python3 experiments/YH009/scripts/jquants_fetch.py --codes 7267        # Honda だけ
python3 experiments/YH009/scripts/jquants_fetch.py --skip fins_summary listed_info
```
...
```bash
python3 experiments/YH009/scripts/car_engine.py
```

出力:
- `experiments/YH009/data/parsed/tape/legs_computed.csv` — ADV20/60, market_cap 等
- `experiments/YH009/data/parsed/tape/legs_car.csv` — 8 CAR 列
- `experiments/YH009/data/parsed/tape/car_report.md` — サマリ + G004(Honda)/G008(Nintendo) 詳細

## 5. G004 / G008 の CAR を手計算と突合

`car_report.md` の下部に G004/G008 の詳細が出ています。この数字を手計算 (Excel + J-Quants 生 CSV) と突き合わせて一致することを確認。

不一致なら `experiments/YH009/PREREG.md` の day 0 規則・estimation window を確認し、必要なら `experiments/YH009/configs/car.yaml` を調整して再実行。
...
vim experiments/YH009/configs/car.yaml
# model.primary を 'topix_adjusted' → 'market_model' に変更、または逆

python3 experiments/YH009/scripts/car_engine.py    # 再実行
```

**本採用の仕様は `experiments/YH009/PREREG.md` に確定させてから運用してください。**
