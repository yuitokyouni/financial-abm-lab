...
- Task A entrypoint: `experiments/YH009/scripts/fetch_jpx_offauction.py` (単体で完結)
- Task A config: `experiments/YH009/configs/jpx_offauction.yaml`
- Task A cron 例: `experiments/YH009/cron/jpx_offauction.crontab`
- Task B pipeline: `migrate_xlsx_to_csv.py` → `archive_pdfs.py` → `validate_tape.py` → `build_tape.py`
- Task B canonical CSVs: `experiments/YH009/data/parsed/tape/{groups,legs,sources}.csv, lists.yaml`
...
- **他コードにimport依存させない**: Task A スクリプトは `experiments/YH009/scripts/` の内部だけを import する。`packages/`, `src/fabm` からも独立。
