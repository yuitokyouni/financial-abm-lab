...
python3 experiments/YH009/scripts/jquants_fetch.py      # 新規銘柄を自動追加取得(15-20分)
python3 experiments/YH009/scripts/shortfall_engine.py   # s1/s2/s3 → data/parsed/tape/legs_shortfall.csv
python3 experiments/YH009/scripts/car_engine.py         # 系統A CAR(任意)
```
...
python3 experiments/YH009/scripts/edinet_fetch.py        # step1 候補抽出(値決め書類 ord=010×docType{030,040,100,190})
python3 experiments/YH009/scripts/edinet_classify.py     # step2 本文DL+株式売出抽出(tier2)
python3 experiments/YH009/scripts/edinet_to_worksheet.py # step3 offering単位集約+include列つき下書き
#   ↑の下書き include 列を人がレビュー(下記スクリーニング)
python3 experiments/YH009/scripts/edinet_merge.py --apply # step4 include=Y を Tier2_candidate で追記(発行体×値決め日で重複ガード)
```
...
- **単体スクリプト**: `experiments/YH009/scripts/fetch_jpx_offauction.py`
- **設定**: `experiments/YH009/configs/jpx_offauction.yaml` (`expected_columns`/`expected_headers` が schema-change 検知の宣言アンカー)
...
python3 experiments/YH009/scripts/fetch_jpx_offauction.py

# 2. crontab に登録 (dry-run で内容を見て、OKなら --install)
bash experiments/YH009/cron/setup_macos_cron.sh                # 何が入るか見るだけ
bash experiments/YH009/cron/setup_macos_cron.sh --install      # 実際に crontab -e 相当を実行
...
2. **スリープ対策** (以下いずれか):
   ...
   - launchd 版に切り替え (`bash experiments/YH009/cron/setup_macos_launchd.sh --install`) — スリープでロスした発火を起床時にキャッチアップする
...
bash experiments/YH009/cron/setup_macos_launchd.sh --install
...
python3 -m pytest experiments/YH009/tests/test_car_engine.py -v         # 30 件通ることを確認
export JQUANTS_API_KEY="ダッシュボードのAPIキー"                    # V2 認証
python3 experiments/YH009/scripts/jquants_fetch.py                       # 15-20 分
python3 experiments/YH009/scripts/car_engine.py                          # 秒単位
# → experiments/YH009/data/parsed/tape/car_report.md を開いて G004/G008 の値を手計算と突合
```
...
- **spec**: `experiments/YH009/MEASUREMENT_SPEC.md`
- **engine**: `experiments/YH009/scripts/shortfall_engine.py` → `data/parsed/tape/legs_shortfall.csv`
...
### Mac での実行
```bash
python3 experiments/YH009/scripts/jquants_fetch.py   # 済ならスキップ (Open=O も取得済み)
python3 experiments/YH009/scripts/shortfall_engine.py
cat experiments/YH009/data/parsed/tape/legs_shortfall.csv
```
...
- **spec**: `experiments/YH009/BENCHMARK_SPEC.md`(ユーザ確定 v0.1 + 実装ノート)
- **engine**: `experiments/YH009/scripts/benchmark_engine.py`
...
python3 experiments/YH009/scripts/benchmark_engine.py --fetch
# 2) 参照分布を計算
python3 experiments/YH009/scripts/benchmark_engine.py
cat experiments/YH009/data/parsed/benchmark/benchmark_report.md
```
...
1. `experiments/YH009/inputs/pdfs_supplied/S0XX__{descriptive_name}.pdf` に配置
2. `experiments/YH009/inputs/pdfs_supplied/manifest.jsonl` に 1 行追記 (sha256, source_id, kind=`pdf_downloaded_then_pinned` or `pdf_supplied_by_user`)
