...
> ① 差分表(研究対象/標的データ/識別戦略)のどの行を毀損するか ② 退化経路 D1(単一ベニュー化)/D2(SF回帰)/D3(応答検証後回し)に近づくか ③ 凍結spec(s1/s2/s3・IS_adj・Nゲート・s3のroute間比較禁止)と矛盾しないか。いずれか YES なら一時簡略化と明示するか設計を戻す(恒久化不可)。
