...
KW = ["売出"]
IPO_KW = ["新規公開", "新規上場"]
TARGET = ["030", "040", "100", "190"]   # 180 は work-list から除外(値決めの 190 で捕捉)
ORDS = ["010"]
...
def test_extract_filters_doctype_ordinance_withdrawal():
    results = [
        ...
        _r(docID="J", docTypeCode="180", ordinanceCode="010"),  # 臨時報告書(発表側) → work-listから除外
        _r(docID="K", docTypeCode="190", ordinanceCode="010"),  # 訂正臨時報告書(値決め) → 拾う
    ...
    assert ids == {"A", "C", "F", "K"}   # 180(J)は work-list 対象外
