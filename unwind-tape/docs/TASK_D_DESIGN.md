...
- method 別に measurable legs をカウントし、**≥30・主要方式2系統×各≥10** を追跡(Task D report に常時出力)。
  カウントは **A_explicit 主 / A+B 感度**の2系列で出す(§確定事項2)。
...
1. **対象期間 = 2022–2026(確定)**。母集団はこの5年の売出し系書類を EDINET で走査。
2. **confidence tier(既定・可逆)**: **A_explicit / B_inference を両方記録し `confidence_policy_holding` 列で区別**。
   N ゲートは **"A_explicit のみ" を主カウント**、**"A+B 合算" を感度分析**として両方 Task D report に出す。
   → B を捨てないので tier 境界は後からいつでも切替可(水増しにならない=正直さ規律に整合)。
