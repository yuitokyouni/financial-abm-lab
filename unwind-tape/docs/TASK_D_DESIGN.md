...
- **第2方式 ≥10 が最大の難所**: 立会外は無帰属の壁で measurable 化しにくく、offering 一辺倒だと
  「主要方式2系統×各10」を満たせない可能性。→ 早期に method 別カウントを監視し、届かないなら
  **N ゲート定義(方式の括り方)の見直しを人間判断に回す**(勝手に緩めない)。
...
1. **対象期間**(母集団に含める年。例 2022–2026)。
2. **confidence tier**: `B_inference` を N ゲートに含めるか、`A_explicit` のみか。
3. **第2方式が ≥10 に届かない場合の代替**(方式の括り直し / N の緩和 / offering 内の下位分類)。← 事前に方針だけ決める。
4. **EDINET API**: v2 は登録制(サブスクリプションキー)。キー取得を誰がやるか。 [要確認: API 仕様の最新版]
