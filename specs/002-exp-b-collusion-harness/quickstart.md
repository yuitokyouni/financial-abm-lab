...
## 検証（gate 分類器・機構・benchmarks・sanity）

```bash
uv run pytest tests/test_benchmarks.py tests/test_env_mechanics.py \
              tests/test_verdict_gate.py tests/test_qlearn_sanity.py -q
```

**ここが緑 = B の測定装置（分母・gate・機構 ablation）が信用できる状態**。001 の battery（既存 6 ファイル）も含め全 suite 緑を維持する。
...
from microstructure.verdict import measure, impulse_response, certify
...
results = [train(cfg.replace(seed=s)) for s in range(5)]
