...
- **Decision**: **本 feature の総予算 = 3×10⁹ 学習期**（runner が累計をカウントし、超過する run を起動拒否）。配分：
  ...
  - **改定（2026-06-11, claim-first 再フレーミング）**: headline 点の定義を変更。主成果物が Calvano 移植可能性監査になった（research-design.md §9）ため、headline = **density spoke の認定セル**（存在すれば）であり、Tier-3 は density regime 上で執行する。認定ゼロの場合は「基準 v2 でも非収束が algo/HP 横断で再現する」ことの頑健性検査に充てる（監査主張の直接証拠）。baseline 疎度セルを headline にする旧想定は finding 0002 により失効。
